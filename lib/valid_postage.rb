...
class ValidPostage
  ...
  def calculate_combinations(current_depth, applied_postage, working_denoms)
    ...
    puts "#{prefix}Applied #{applied_postage}. Checking #{working_denoms.first}" if verbose

    if calculations_complete
      puts "#{prefix}| Nothing more can make postage on this chain" if verbose
      return
    end

    if working_denoms.empty?
      puts "#{prefix}| Denoms all checked on this chain" if verbose
      return
    end
    ...
    if current_depth > stamp_limit
      ...
      print "#{prefix}| We are out of stamp slots" if verbose
      if applied_postage.first == active_denom && !valid_denoms[active_denom]
        print "#{prefix}| and shouldn't check any further down this chain" if verbose
        self.calculations_complete = true
      end
      puts if verbose
      return
    ...
    if new_postage.sum >= target_postage # We made postage,
      puts "#{prefix}| We can make postage this level" if verbose
      ...
      puts "#{prefix}> Let's add another stamp!" if verbose
    ...
    breadth_copy.shift
    puts "#{prefix}> Let's check the next denomination!" if verbose
  ...
  def format_summary
    within_overpay = valid_postage.select { |e| e.sum - target_postage <= overpay_limit }
    counts = {}
  ...
  def parse
    OptionParser.new do |parser|
      ...
      parser.on('-c', '--csv', 'Output csv format') do |csv|
        self.csv = csv
      end
